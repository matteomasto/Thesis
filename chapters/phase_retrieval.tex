...
\begin{table}[H]
    ...
    \resizebox{\linewidth}{!}{%
      \begin{tabular}{|l|l|}
        ...
        \texttt{update\_border\_n}                       & \texttt{0} \\
...
conventional iterative algorithms and standard routines can be challenging. Fine parameter tuning operated by expert 
scientists is often required, together with large numbers of runs, to achieve successful reconstructions. On the contrary, 
the DL approach is limited to low-resolution reconstructions and, though fast during inference, only provides an output 
drawn from a general approximation of a statistical distribution. For this reason, the DL model for the phasing of 
experimental data needs be used as a preprocessing step that can provide 
...
the convergence, assumed that the DL estimate is located near the solution, in the search space. In this framework 
the DL model is crucial for the finding of the right neighborhood to which the solution belongs to, therefore drastically 
reducing the computational efforts for the search of the solution space, hence eventually the wall-clock time.
...
the predicted object to the size relative to the original dataa one. This step is fundamental when the DL object is then 
