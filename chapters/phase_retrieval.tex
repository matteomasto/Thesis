...
the experimental condition. A set containing 30'000 samples was created and the ``bigger'' model mentioned in Fig 
...
\begin{figure}[H]
    ...
    \caption{Corresponding reconstructed objects. } 
...
the average of the overlapping prediction was implemented the issue does not seem to be fully resolved.
...
(ii) the stitching is problematic already from the first shell, most likely because of the two issues pointed out above (
    initial guess from model prediction and overwriting of the new patches)\\
...
\begin{figure}[H]
    ...
    \caption{Predicted object's modulus before the zero-frequency component filtering compared with the ground truth one. }
