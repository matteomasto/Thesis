...
encoder-decoder was implemented, and the model was trained for 10 epochs in a supervised fashion with a cross-entropy loss function.
...
within a few milliseconds on a desktop machine. 
...
DIPs, one for the modulus and one for the phase, and successfully manages to reconstruct simulated objects even in the presence 
of strong phases. 
...
enhance the training. This is rather peculiar as this kind of residual links are typically used, in convolutional 
...
parameters, hence the prediction. The first work that employs a cross entropy loss, normally 
used for classification tasks, whereas other works opt for Mean Absolute Error (MAE) and Mean Squared Error (MSE), which are standard 
for regression tasks and Pearson Correlation Coefficient (PCC). Typically, 
...
Lastly, I have listed the different ways used to refine the DL predictions. Here we can notice that GPU accelerated
gradient descent methods have been used to replace of conventional iterative algorithms. The unsupervised training
...
thus interfering like a single slit, with the typical fringes of intensity that correspond to constructive interference, 
interspersed with dark regions arising from destructive interference. In these specific cases, the RSP shows a regular 
...
Given this ``simple'' law of constructive-destructive interferences, we hypothesized the possibility to predict patches 
of this RSP given a portion of diffraction pattern and then, similarly to the inpainting case, stitch them together
and obtain the full RSP. This entails a number of complications related to the so-called phase symmetries (see Section \ref{sec:phase_symm}) 
...
I have trained the model in a supervised manner, meaning, in this case, that the training was always conducted on simulated data 
...
\begin{figure}[H]
    ...
    \caption{\textbf{Different simulated strain distributions.}  Examples of each of the three adopted methods to simulate 
...
Similar to the approach for the inpainting case, I have first conducted some preliminary studies in 2D, on noise-less low strain data. Here I will 
...
\begin{figure}[H]
    ...
    and same LeakyReLU activation. Skip connections are employed as well between encoder and decoder. Copies of the 
    ...
    The total number of trainable parameters is of the order of 6.8 million. }
...
derived from the incorrect superposition of the correct shape and its twin, also a correct solution since an object and 
its conjugate symmetric have the same Fourier transform amplitude. More generally, it seems 
...
\begin{figure}[H]
    ...
    \caption{\textbf{Model testing using MSE loss function and biased dataset}. Here, the training dataset was biased 
    with the method proposed in \cite{zhang_what_2024} in order to break the RSP sign symmetry. First row shows four 
    simulated BCDI patterns, second row the ground truth RSP 
...
the modulo wrapping function jumps abruptly by 2$\pi$ every time the phase crosses an integer multiple of 2$\pi$, meaning that 
...
To do such, it is necessary to express $\Delta\varphi_k$ as angles of a complex exponential. This means that if $\varphi_{pred}$ is an array of 
...
is an efficient metric for the measurement of the \textit{degree of alignment} of all the $z_k$ between each other. 
...
\begin{figure}[H]
    ...
    in a $\varphi_{pred}$ shifted by a constant offset. The offset has been removed before plotting in \textbf{b} for 
...
\begin{figure}[H]
    ...
    is less pronounced, the model seems to manage the two solutions. }
    \label{fig:obj_vfn}
\end{figure}
...
in section \ref{sec:dataset_creation2D}. In this case Poisson statistics was applied to each dataset to better simulate 
...
\begin{figure}[H]
    ...
    \caption{Reconstructed objects corresponding to Fig \ref{fig:2D_hs_VFN_RSP}. Compared to the objects obtained 
...
Three-dimensional BCDI patterns of low strained particles were used to conduct this study. 
...
of the reciprocal space, and in particular if it can be retrieved by a DL model. 
Moreover, the use of patches is further motivated by the local behavior of the phase, which typically oscillates in an 
approximately linear fashion from $0$ to $2\pi$, mirroring the intensity fringes. This local periodicity implies a 
degree of spatial consistency and tractability that justifies the use of localized patches for learning and reconstruction.
(From now on the term ``patches'' will be used to refer to cubic subsets of the reciprocal space). \\
...
\begin{itemize}
    ...
    \item An even number multiple of 2 is usually considered GPU-friendly since it facilitates shared calculations across
    different threads. 
\end{itemize}
...
However, by doing so the RSP of each patch would in principle have different offsets and different wraps than their neighbors 
...
To summarize, the final design implied the use of two distinct training datasets and two different CNNs as depicted in 
Fig.\ref{fig:2models}.
...
\begin{figure}[H]
    ...
    this initial RSP guess is taken from the ground truth simulated RSP while during inference this is taken from the 
    ...
    stitched with the previous predictions (\textbf{e}) to form the full RSP. The process is repeated until the 
...
The total number of trainable parameters is of the order of 3.5 million. \\
...
space, hence wrong zero RSP values\footnote[1]{For zero values of the RSP in the center of the object we have 
$O(\mathbf{r} = 0) = \sum_{\mathbf{q}} \sqrt{I(\mathbf{q})}e^{i\varphi(\mathbf{q})} = \sum_{\mathbf{q}} \sqrt{I(\mathbf{q})}$ 
hence an incorrect constructive interference of zero-RSP contributions that leads to the central spike.}. Fig. \ref{fig:stitching_filtering} shows the object's modulus before the removal of 
...
Although it failed, this study on the prediction of the RSP in smaller patches has led to a better understanding of the problem 
...
Similarly to the previous cases, the approach was to create a dataset to train the model with, in a supervised manner with 
...
The model correctly predicts the RSP oscillations inside the fringes even when they have been distorted or merged into
...
the object rather than a clean but inaccurate one. Moreover, in experimental conditions the shape may be known via other 
...
(Particle 1) \cite{data1} while the second (Fig. \ref{fig:exp_RSP2} b) is a dewetted platinum/palladium bilayer on a sapphire substrate
(Particle 2) \cite{data2}. 
...
\begin{figure}[H]
    ...
    \caption{Phase of the three best reconstructions of Particle 2. The low reproducibility of the result highlighted by 
    the diversity of the reconstructions and supported by the relatively low first SVD eigenvalue, confirms the poorly accurate 
    reconstruction shown in Fig.\ref{fig:pynx_rec2}.}
...
to crop and resize the experimental diffraction patterns. In fact, especially for large BCDI data, a 
resizing or binning step before cropping is often needed, such that a sufficient amount of data is contained in the 64 pixel-size 
...
Due to the large diversity of the region of interest (ROI) from one experimental BCDI pattern to the other, the manual 
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{figures/Phasing/projs_part2.pdf}
...
In addition to the higher quality of the results it is worth mentioning the significant reduction of the wall-clock time. 
...
In this section the results of the DL model presented previously, tested against different strain configurations and 
...
objects have been obtained with inverse FT. At this point the accuracy of the prediction was calculated using the 
...
\begin{figure}[H]
    ...
    random Gaussian fields. One can see a significant drop of the accuracy with respect to Figs. \ref{fig:gauss} 
    ...
    the higher accuracy obtained previously. }
...
model predicted objects have been used as a starting point for 300 ER refinement, always with PyNX and the parameters in 
...
the object's phase is simulated with cosine functions. In this case as well, the DL model, although when used alone yields relatively low accuracy scores, improves the quality of the 
...
and training samples. A possible explanation is given by the symmetry of the diffraction pattern used as input. The averaged 
...
\begin{figure}[H]
    ...
    \caption{Example of DL prediction for a low strain randomly shaped particle. The non-centrosymmetricity of the object's 
...
and loss function were adopted like those presented in the paper, and tests on the same data shown in Fig. \ref{fig:obj_paper_sim} 
have been performed with results shown in Fig. \ref{fig:Ian_objs}. 
...
inferred with this model (see Fig. \ref{fig:Ian_rec1}) and the result used as a starting point for further refinement 
...
For this reason, overall, the predicted object does not represent a close enough estimate for ER to converge to the 
...
\begin{itemize}
    ...
    speeds up the training as it takes less time than a loss computed in real space. Moreover, the use of the WCA avoids 
    ...
    results in 2D and 3D. It has proven to be successful at inverting experimental data that happened to be extremely challenging with 
    ...
    \item Lastly, the DL model proved to be compatible with iterative refinement using conventional algorithms, contributing to 
