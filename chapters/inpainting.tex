...
would be there, before the phase retrieval. The task of filling gap in images is usually referred to as ``inpainting''.
...
As we have seen in \ref{chap:dl_theory:intro}, the typical CNN architecture for image generation consists of an encoder, which
...
\begin{figure}[h]
    ...
    patches. \textbf{b)} $32\times32\times32$ pixel-size sub-volumes with 9 pixel-wide vertical and cross-shaped gaps.
...
Figure \ref{fig:Carnis_obj} illustrates instead the central slice of the reconstructed objects for 
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.52\textwidth}
        ...
        \caption*{}
    \end{subfigure}
    \caption{\textbf{RMSE of the strain field versus gap size} }
...
\begin{figure}[H]
    ...
    \caption{Zoom on the projection along the rocking curve axis of the Pt BCDI pattern calculated from the reconstructed object 
    obtained with PyNX software using \textbf{(a)} a mask on the gaps, \textbf{(b)} a mask on the streaks only, leaving zeros 
    inside the gaps \textbf{(c)} and inpainting the gaps with our DL model \textbf{(d)}. Inside the regions where the mask 
    was applied the intensity is free to vary throughout the PR iterations. }
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.47\textwidth}
        ...
        \caption*{}
    \end{subfigure}
    \caption{Modulus and strain of the object's reconstructions for each case mentioned above in Fig. \ref{fig:high_res_Icalc}. 
    The oscillatory artifacts are smallest for the object obtained after DL inpainting.}
