...
The final training dataset consisted of 50'000 $32\times32\times32$ sub-volumes created as described above. 

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/Inpainting/process.pdf}
...
Given the reduced size of the inputs, the encoder this time is composed of four blocks only, after in each of which there
are convolutional layers and maximum pooling layers. The feature map is thus reduced to a $2\times2\times2\times478$ tensor 
...
\begin{figure}[h]
    ...
    \caption{Schematic of the 3D model architecture.}
    \label{fig:architecture3d}
\end{figure}
