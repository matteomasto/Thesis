...
pointing at the Bragg peak, we could draw a sphere of radius $k$ centered in sample position and find the diffraction pattern 
of interest at the intersection of such sphere with the reciprocal lattice in the $hkl$ node. In other words, the diffraction 
from a wave-vector $\mathbf{k}$ occurs only in those reciprocal lattice points that intersect the sphere centered in the sample 
with radius $k$. Such sphere is called \textit{Ewald sphere} and it is illustrated in Fig.\ref{fig:ewald}. 
It follows that the detector is placed tangentially to the Ewald sphere in 
% The surface of the sphere around the Bragg peak tells us
