...
% From this point of view one could classify different experimental cases depending on the number of voxels involved in 
% large phase variations, the magnitude of the variations and the arrangement inside the object of those voxels. 


% It follows that the more are the voxels involved in incoherent phase variations, the more speckled is 
% the diffraction pattern, as each contribution is moving away from the original Bragg condition differently. This is 
% often called \textit{heterogeneous} strain. On the contrary, we can consider the case of equal displacements with 
% respect to the perfect lattice across many lattice points. Here, there is a \textit{coherent} summation of the 
% contributions, that leads to the so-called \textit{homogeneous} strain. The result is therefore the splitting of the 
% Bragg peak into two peaks, one given by the pristine lattice and one by the 


% The presence of phase variations across voxels is often referred to as \textit{heterogeneous} strain, 
% and it is responsible for the breaking of the diffraction pattern into speckles.

% In particular, the contribution to the diffraction pattern of a $N$ voxels with large phase is 
% represented by an amount of signal proportional 
...
% STRAIN AND INCOHERENCY - QUALITATIVE
% PHASE RANGE AND LOCAL MINIMA -->
