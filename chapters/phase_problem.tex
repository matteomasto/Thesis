...
Eq. \ref{eq:modulus_projection} can be rewritten including the inverse Fourier transform that maps the object into direct 
space after the projection: 

\begin{equation}
    \mathcal{P}_{Cm}(O(\mathbf{r})) = \mathcal{F}^{-1}\{ \sqrt{I(\mathbf{q})}e^{i\text{ }\text{arg}(\mathcal{F}\{ O(\mathbf{r})\})}\}
    \label{eq:modulus_projection}
\end{equation}
